...
def _unique_modern_custom_provider_entry(
    providers: object, provider_id: str
) -> dict | None:
    """Return one modern ``providers`` mapping entry for ``custom:<slug>``.

    WebUI-managed providers normally use ``custom:<slug>`` as the mapping key,
    while older hand-written config may use the bare slug. Treat both as the
    same identity and fail closed if both forms coexist, so auxiliary-model
    persistence never combines one entry's endpoint with another identity.
    """
    if not isinstance(providers, dict) or not provider_id.startswith("custom:"):
        return None
    wanted = _canonicalise_provider_id(provider_id)
    matches: list[dict] = []
    for raw_key, entry in providers.items():
        ...
        if not key:
            continue
        candidate = key if key.lower().startswith("custom:") else f"custom:{key}"
        if _canonicalise_provider_id(candidate) == wanted:
            matches.append(entry)
    if len(matches) >= 2:
        raise AmbiguousCustomProviderError(
            f"Multiple providers mapping entries resolve to {provider_id!r}; "
            "remove the duplicate bare/custom key before saving this model."
        )
    return matches[0] if matches else None
        ...
        with active_profile_transaction(lambda: _get_config_path().parent) as profile_home:
            ...
            if task == "__reset__":
                ...
                if provider and (provider.startswith("custom:") or provider == "custom"):
                    ...
                    # provider, not the active main provider. All lookups use the
                    # transaction's already-loaded config snapshot, so this path
                    # never re-enters the non-reentrant config lock.
                    resolved_base_url = None
                    if provider.startswith("custom:"):
                        # Prefer the modern WebUI-managed ``providers`` mapping,
                        # then fall back to legacy ``custom_providers``.
                        _cp_match = _unique_modern_custom_provider_entry(
                            config_data.get("providers", {}), provider
                        )
                        if _cp_match is None:
                        ...
                        if _cp_match is not None:
                            resolved_base_url = str(
                                _cp_match.get("base_url")
                                or _cp_match.get("url")
                                or _cp_match.get("api")
                                or ""
                            ).strip() or None
                    if not resolved_base_url:
                        # Best-effort fallback for the unnamed `custom` case.
                        # Genuine identity ambiguity remains write-blocking.
                        try:
                ...
                try:
                    ...
                    # Always run the secret planner: even without a new
                    # advanced-options payload it migrates a legacy literal
                    # api_key out of config.yaml into the owned .env reference.
                    env_updates = _plan_advanced_model_secret(
