...
这是一个 WebUI 扩展包（见 `docs/EXTENSIONS.md`），界面与数据层都通过既有扩展挂载点接入，
并复用 WebUI 的工作区文件、上传、语音转写和临时 Agent 请求接口。不想使用时，把 `.env`
里的两行配置注释掉即可。
...
| 模块 | 用途 |
| --- | --- |
| **主页** | 问候与今日聚焦、工作台统计、快速捕获、跨模块最近动态 |
| **会议纪要** | 浏览器录音/音频导入、语音转写、AI 总结与历史回看 |
...
索引数据是 Hub 根目录下的纯 JSON，可以直接编辑、git 版本管理和备份；会议原始录音
单独放在 `hub-recordings/`：

```
hub-profile.json     个人档案与今日聚焦
hub-meetings.json    会议元数据、转写文本与总结
hub-design.json      产品设计条目
hub-ops.json         服务与命令
hub-resources.json   资源收藏
hub-inbox.json       收件箱
hub-recordings/      会议原始录音（WebM / OGG / M4A 等）
