...
把 Hermes WebUI 变成个人工作中枢：需求、会议、服务、资源、灵感都在这里管，
...
| 模块 | 用途 |
| --- | --- |
| **主页** | 问候与今日聚焦、会议/行动项等全局统计、快速捕获、跨模块最近动态 |
| **产品设计** | 五阶段看板：想法 → 需求 → 设计中 → 评审 → 已交付 |
| **会议** | 会议纪要、决策、风险、待确认问题，以及带负责人/截止时间/交付物/验收标准/依赖的行动项 |
...
而不是白屏。`hub-meetings.json` 还会规范旧版或 Agent 手改的嵌套字段；保存前按会议
`id` 与磁盘最新版本做三方合并，同一会议发生并发冲突或文件 JSON 已损坏时会拒绝覆盖。
